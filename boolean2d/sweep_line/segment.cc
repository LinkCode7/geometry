...
Segment2d::Segment2d(const segment_2d& segment2d) : m_segment2d(segment2d), m_sweepLineCrossPoint(m_segment2d.first)
{
...
{
    return abs(bg::get<0>(this->m_segment2d.first) - bg::get<0>(other.m_segment2d.first)) < EPS &&
           abs(bg::get<1>(this->m_segment2d.first) - bg::get<1>(other.m_segment2d.first)) < EPS &&
           abs(bg::get<0>(this->m_segment2d.second) - bg::get<0>(other.m_segment2d.second)) < EPS &&
           abs(bg::get<1>(this->m_segment2d.second) - bg::get<1>(other.m_segment2d.second)) < EPS;
}

// 默认从左下到右上
bool Segment2d::operator<(const Segment2d& other) const
{
    if (bg::get<0>(this->m_segment2d.first) != bg::get<0>(other.m_segment2d.first))
        return bg::get<0>(this->m_segment2d.first) < bg::get<0>(other.m_segment2d.first);
    else if (bg::get<1>(this->m_segment2d.first) != bg::get<1>(other.m_segment2d.first))
        return bg::get<1>(this->m_segment2d.first) < bg::get<1>(other.m_segment2d.first);
    else if (bg::get<0>(this->m_segment2d.second) != bg::get<0>(other.m_segment2d.second))
...
std::ostream& operator<<(std::ostream& out, Segment2d& other)
{
...
// segment_2d Segment2d::GetOriginSegment() const
//{
//     segment_2d segment2d = m_segment2d;
//     segment2d.first      = m_originStartPoint;
//     return segment2d;
// }
